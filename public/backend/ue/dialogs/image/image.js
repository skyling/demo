...
(function() {
    ...
    RemoteImage.prototype = {
        ...
        initEvents: function() {
            ...
            domUtils.on($G("method"), 'change', function() {
                if ($G("method").checked) {
                    $G('width').disabled = true;
                    $G('height').disabled = true;
                    $G('percent').disabled = false;
                    $G('method_size').setAttribute("style", "display:none;");
                    $G('method_percent').setAttribute("style", "");
                } else {
                    $G('width').disabled = false;
                    $G('height').disabled = false;
                    $G('percent').disabled = true;
                    $G('method_percent').setAttribute("style", "display:none;");
                    $G('method_size').setAttribute("style", "");
                }
            });
            ...
            domUtils.on($G("percent"), 'keyup', function() {
                if($G("percent").value <0) {
                    $G("percent").value = '';
                }
                if($G("percent").value > 100 ) {
                    $G("percent").value = 100;
                }
                $G('width').value = editor.body.clientWidth*(($G('percent').value)/100);
                var image = new Image();
                image.src = $G('url').value;
                image.onload = function(){
                    $G('height').value = $G('width').value/(image.width/image.height);
                };
        ...
        setImage: function(img) {
            ...
            if (src) {
                ...
                $G("percent").value = Math.round(img.width/editor.body.clientWidth*10000)/100;
                setAlign(align);
                this.setPreview();
                this.updateLocker();
            }
        ...
        setPreview: function() {
            ...
            if (method.checked) {
                width = percent.value>0 ? (percent.value+"%") : '';
                height = "";
            }
        ...
        getInsertList: function() {
            ...
            // 百分比
            if (this.dom.method.checked) {
                data['width'] = this.dom.percent.value+"%";
                data['height'] = 'auto';
            } else {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function() {
            var _this = this,
                ...
                // WebUploader实例
                uploader,
                actionUrl = editor.getOpt("qiniuUploadUrl") || editor.getActionUrl(editor.getOpt('imageActionName')),
            ...
            setState('pedding');
            // 获取七牛token
            if (editor.getOpt("qiniuToken")) {
                var token = editor.getOpt("qiniuToken")(false);
                if (!uploader.options.formData) uploader.options.formData = {};
            ...
            uploader.on('uploadSuccess', function(file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS' || (editor.getOpt('qiniuUploadUrl')) && !!json.key) {
                        if ((editor.getOpt('qiniuUploadUrl')) && !!json.key) {
                            json.url = editor.getOpt("qiniuUrlPrefix") + json.key;
                        }
            ...
            uploader.on('uploadError', function(file, code) {
                // 刷新七牛token
                editor.getOpt("qiniuToken") && editor.getOpt("qiniuToken")(true);
            });
